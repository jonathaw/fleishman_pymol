...
def load_int_models(files,obj,discrete=0):
  """
  load_int_models <files>, <object>, <discrete=0>
  ...
  e.g. load_int_models prot_*.pdb, prot
  """
  file_list = glob.glob(files)
  if file_list:
    file_list.sort()
    for name in file_list:
      loadInterfacePDB(name,obj)
  else:
    print "No files found for pattern %s" % files

cmd.extend('load_int_models',load_int_models)
